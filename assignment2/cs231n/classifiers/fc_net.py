...
class TwoLayerNet(object):。‘
    ...
    def loss(self, X, y=None):
        ...
        #cache1= fc_cache(X, W1, b1), hidden_in
        scores, cache2 = affine_forward(hidden_out, W2, b2)
        #cache2 = hidden_out, W2, b2
    ...
    def loss(self, X, y=None):
        ...
        for lay in range(self.num_layers - 1):
            ...
            if self.normalization is None:
                layer_input, ar_cache[lay] = affine_relu_forward(layer_input, w, b)
            else:
                gamma = self.params['gamma%d' % (lay + 1)]
                beta = self.params['beta%d' % (lay + 1)]
                if self.normalization == 'batchnorm':
                        layer_input, ar_cache[lay] = affine_bn_relu_forward(layer_input,w, b, gamma, beta, self.bn_params[lay])
                elif self.normalization == 'layernorm':
                        layer_input, ar_cache[lay] = affine_ln_relu_forward(layer_input,w, b, gamma, beta, self.bn_params[lay])
                else:
                        ar_cache[lay] = None
        ...
        #reg_loss_final = 0.5 * self.reg * (np.sum(W_final * W_final))
        #loss = data_loss + reg_loss_final
