...
def dropout_forward(x, dropout_param):
    ...
    NOTE 1: Please implement **inverted** dropout, not the vanilla version of dropout.
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_out = int((H - pool_h) / stride) + 1
    W_out = int((W - pool_w) / stride) + 1
    ...
    for data_point in range(N):
        for channel in range(C):
            for hh in range(H_out):
                for ww in range(W_out):
                    out[data_point,channel,hh,ww] = np.max(x[data_point, channel, stride*hh:stride*hh+pool_h, stride*ww:stride*ww+pool_w])
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for channel in range(C):
            for hh in range(H_out):
                for ww in range(W_out):
                    #mask遮罩，np.max()=1 else=0
                    temp = x[n,channel,stride*hh:stride*hh+pool_height,stride*ww:stride*ww+pool_width]
                    mask = (temp==np.max(temp))
                    dx[n,channel,stride*hh:stride*hh+pool_height,stride*ww:stride*ww+pool_width] = dout[n,channel,hh,ww] * mask
